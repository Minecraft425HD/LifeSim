...
public class World {
    ...
    public void tick(){
        ...
        double rg=0.06*(currentSeason==Season.SPRING?2.2:currentSeason==Season.WINTER?0.25:1.0);
        for(double[] f:foodList) if(f[2]<f[3]) f[2]=Math.min(f[3],f[2]+rg);
    }
    public double consumeFood(double x,double y,double r){
        double tot=0;
        for(double[] f:foodList){double d=dist(x,y,f[0],f[1]);if(d<r&&f[2]>0){double e=Math.min(f[2],5.0*(1-d/r));f[2]-=e;tot+=e;}}
        return tot;
    }
    public double nearestFoodDist(double x,double y){
        double best=width;for(double[] f:foodList)if(f[2]>0.5)best=Math.min(best,dist(x,y,f[0],f[1]));return best;
    }
    public double[] nearestFoodPos(double x,double y){
        double best=Double.MAX_VALUE;double[] bf=null;
        for(double[] f:foodList){if(f[2]<0.5)continue;double d=dist(x,y,f[0],f[1]);if(d<best){best=d;bf=f;}}
        return bf!=null?new double[]{bf[0],bf[1]}:null;
    }
    public double nearestDangerDist(double x,double y){
        double best=width;for(double[] d:dangerList)best=Math.min(best,dist(x,y,d[0],d[1]));return best;
    }
    public double dangerDamage(double x,double y){
        double tot=0;for(double[] d:dangerList){double dd=dist(x,y,d[0],d[1]);if(dd<d[2])tot+=3.0*(1-dd/d[2]);}return tot;
    }
    ...
    public static double dist(double x1,double y1,double x2,double y2){double dx=x1-x2,dy=y1-y2;return Math.sqrt(dx*dx+dy*dy);}
