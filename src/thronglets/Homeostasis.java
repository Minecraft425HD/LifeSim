...
public class Homeostasis {
    ...
    public Homeostasis(Random rng){
        for(DriveType d:DriveType.values())
            drives[d.id]=d.optMin+rng.nextDouble()*(d.optMax-d.optMin);
    ...
    public void tick(Season s,boolean inGroup,double moved,boolean atFood){
        ...
        drives[0]-=DriveType.ENERGY.decayRate*s.energyCost*cfg.energyDecayFactor;
        ...
        double base = (s==Season.WINTER?0.5:s==Season.AUTUMN?0.2:-0.1);
    ...
    public void applyDanger(double d){
        drives[0]-=d;
        drives[1]=Math.min(100,drives[1]+d*2);
    }

    public void applySocial(boolean p){
        drives[2]=Math.min(100,drives[2]+(p?15:-5));
        drives[1]=Math.max(0,drives[1]-(p?5:2));
    }
    ...
    public float[] toInputVector(){
        float[] v=new float[7];
        for(int i=0;i<5;i++)v[i]=(float)(drives[i]/100);
        v[5]=(float)((valence+1)/2);v[6]=(float)arousal;
        return v;
    }
