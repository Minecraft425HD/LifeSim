...
public class NEATGenome {
    ...
    private NEATGenome(){}
    public NEATGenome copy(){NEATGenome c=new NEATGenome();genes.forEach(g->c.genes.add(g.copy()));c.nodeIds.addAll(nodeIds);c.speciesId=speciesId;return c;}
    public void mutate(Random rng){double r=rng.nextDouble();if(r<0.80)mutW(rng);else if(r<0.93)addConn(rng);else addNode(rng);}
    ...
    public static NEATGenome crossover(NEATGenome fit,NEATGenome other,Random rng){NEATGenome c=new NEATGenome();c.nodeIds.addAll(fit.nodeIds);c.nodeIds.addAll(other.nodeIds);Map<Integer,NEATGene> om=new HashMap<>();for(NEATGene g:other.genes)om.put(g.innovation,g);for(NEATGene g:fit.genes){NEATGene o=om.get(g.innovation);NEATGene ch=(o!=null&&rng.nextBoolean())?o.copy():g.copy();if(o!=null&&(!g.enabled||!o.enabled))ch.enabled=rng.nextDouble()>0.75;c.genes.add(ch);}return c;}
